...
<body>
  ...
  private renderStory(story: CrossPlatformStory | Headline): string {
    if ('summary' in story) {
      return this.renderCrossPlatformStory(story as CrossPlatformStory);
    } else {
      return this.renderHeadline(story as Headline);
    }
  }

  private renderCrossPlatformStory(story: CrossPlatformStory): string {
    const links = [];
    if (story.techmemeUrl) links.push(`<a href="${story.techmemeUrl}">Techmeme</a>`);
    if (story.hackernewsUrl) links.push(`<a href="${story.hackernewsUrl}">Hacker News</a>`);
  ...
  private renderHeadline(headline: Headline): string {
